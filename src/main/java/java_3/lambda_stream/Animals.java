...
class Animals {
    ...
    static List<String> capsFirst(boolean mutate) {
        ...
        if(mutate)
        {
            animals = animals.stream().map(n -> n.substring(0,1).toUpperCase() + n.substring(1).toLowerCase()).collect(Collectors.toList());
            return animals;
        }
        else {
            List<String> animalsCopy = animals;
            return animalsCopy.stream().map(n -> n.substring(0,1).toUpperCase() + n.substring(1).toLowerCase()).collect(Collectors.toList());
        }
    }
    ...
    static List<String> lowerFirst(boolean mutate) {
        ...
        if(mutate)
        {
            animals = animals.stream().map(n -> n.substring(0,1).toLowerCase() + n.substring(1).toUpperCase()).collect(Collectors.toList());
            return animals;
        }
        else {
            List<String> animalsCopy = animals;
            return animalsCopy.stream().map(n -> n.substring(0,1).toLowerCase() + n.substring(1).toUpperCase()).collect(Collectors.toList());
        }
    }
    ...
    static List<String> flipAnimals(boolean mutate) {
        ...
        if(mutate)
        {
            animals = animals.stream().sorted(Collections.reverseOrder()).collect(Collectors.toList());
            return animals;
        }
        else {
            List<String> animalsCopy = animals;
            return animalsCopy.stream().sorted(Collections.reverseOrder()).collect(Collectors.toList());
        }
    }
    ...
    //sort the animals in alphabetical order.  If the booleaen parameter is true, mutate the static animals list.  if the mutate boolean is false, sort a 'copy' of the animals list, then return that list of sorted animals, WITHOUT mutating the static animals array. return the sorted list in both cases.
        if(mutate)
        {
            ...
            List<String> animalsCopy = animals;
            return animalsCopy.stream().sorted().collect(Collectors.toList());
        }
    }
